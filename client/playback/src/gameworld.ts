...
export type MapStats = {
  ...
  island_stats: Map<
    number,
    {
      ...
      accelerated_tiles: Set<number>;
      id: number;
    }
  >;

  resources: Int8Array;
  resource_well_stats: Map<
    number,
    { adamantium: number; mana: number; elixir: number; upgraded: boolean }
  >;
...
export default class GameWorld {
  ...
  loadFromMatchHeader(header: schema.MatchHeader) {
    ...
    for (let i = 0; i < this.mapStats.resources.length; i++) {
      if (this.mapStats.resources[i] != 0) {
        this.mapStats.resource_well_stats.set(i, {
          adamantium: 0,
          mana: 0,
          elixir: 0,
          upgraded: false,
    ...
    for (let i = 0; i < this.mapStats.islands.length; i++) {
      if (this.mapStats.islands[i] != 0) {
        ...
        if (this.mapStats.island_stats.has(island_id)) {
          ...
          this.mapStats.island_stats.set(island_id, {
            owner: 0,
            flip_progress: 0,
            locations: [i],
            is_accelerated: false,
            accelerated_tiles: new Set(),
            id: island_id,
          });
        }
      }
        ...
        for (let x = tile_x - acc_radius; x <= tile_x + acc_radius; x += 1) {
          for (let y = tile_y - acc_radius; y <= tile_y + acc_radius; y += 1) {
            if (
              ...
              y >= 0 &&
              y < height &&
              (tile_x - x) * (tile_x - x) + (tile_y - y) * (tile_y - y) <=
  ...
  processDelta(delta: schema.Round) {
    // Change to reflect current game
    if (delta.roundID() != this.turn + 1) {
      throw new Error(
        `Bad Round: this.turn = ${
          this.turn
        }, round.roundID() = ${delta.roundID()}`
      );
    }
    ...
    if (movedLocs) {
      ...
      for (let j = 0; j < movedIds.length; j++) {
        ...
        if (elem.length > 20) elem.pop();
      }
    }

    this.mapStats.effects.forEach((s) => s.turns_remaining--);
    this.mapStats.effects = this.mapStats.effects.filter(
      (s) => s.turns_remaining >= 0
    );

    // Remove abilities from previous round
    this.bodies.alterBulk({
      id: new Int32Array(this.actionRobots),
      action: new Int8Array(this.actionRobots.length).fill(-1),
      target: new Int32Array(this.actionRobots.length),
      targetx: new Int32Array(this.actionRobots.length),
      targety: new Int32Array(this.actionRobots.length),
    });
    this.actionRobots = [];
    ...
    if (delta.actionsLength() > 0) {
      for (let i = 0; i < delta.actionsLength(); i++) {
        ...
        const teamStatsObj =
          body != null ? this.teamStats.get(body.team) : null;
        const width = this.mapStats.maxCorner.x - this.mapStats.minCorner.x;

        const setAction = (
          set_target: Boolean = false,
          set_target_loc_from_body_id: Boolean = false,
          set_target_loc_from_location: Boolean = false
        ) => {
          ...
          if (set_target) this.bodies.alter({ id: robotID, target: target });
            ...
            this.bodies.alter({
              id: robotID,
              targetx: target_body.x,
              targety: target_body.y,
            });
        ...
        switch (action) {
          ...
          case schema.Action.BUILD_ANCHOR:
            if (target == 0) {
              this.bodies.alter({
                id: robotID,
                normal_anchors: body.normal_anchors + 1,
              });
            } else {
              this.bodies.alter({
                id: robotID,
                accelerated_anchors: body.accelerated_anchors + 1,
              });
            }
            break;
          ...
          case schema.Action.PICK_UP_ANCHOR:
            ...
            if (target >= 0) {
              ...
              if (anchor_type == 0) {
                this.bodies.alter({
                  id: hq_id,
                  normal_anchors: hq.normal_anchors - 1,
                });
                this.bodies.alter({
                  id: robotID,
                  normal_anchors: body.normal_anchors + 1,
                ...
                this.bodies.alter({
                  id: hq_id,
                  accelerated_anchors: hq.accelerated_anchors - 1,
                });
                this.bodies.alter({
                  id: robotID,
                  accelerated_anchors: body.accelerated_anchors + 1,
          ...
          case schema.Action.CHANGE_ADAMANTIUM:
            ...
            this.bodies.alter({
              id: robotID,
              adamantium: body.adamantium + target,
            });
            break;
          ...
          case schema.Action.DIE_EXCEPTION:
            console.log(
              `Exception occured: robotID(${robotID}), target(${target}`
            );
            break;

          default:
            console.log(
              `Undefined action: action(${action}), robotID(${robotID}, target(${target}))`
            );
            break;
        }
    ...
    for (let i = 0; i < delta.resourceWellLocsLength(); i++) {
      ...
      let current_resource_stats =
        this.mapStats.resource_well_stats.get(well_index);
    ...
    for (let team in this.meta.teams) {
      ...
      if (statsObj.adamantiumMinedHist.length > averageWindow)
        statsObj.adamantiumMinedHist.shift();
      if (this.turn % 10 == 0)
        statsObj.adamantiumIncomeDataset.push({
          x: this.turn,
          y: average(statsObj.adamantiumMinedHist),
        });

      statsObj.manaMinedHist.push(statsObj.manaMined);
      if (statsObj.manaMinedHist.length > averageWindow)
        statsObj.manaMinedHist.shift();
      if (this.turn % 10 == 0)
        statsObj.manaIncomeDataset.push({
          x: this.turn,
          y: average(statsObj.manaMinedHist),
        });

      statsObj.elixirMinedHist.push(statsObj.elixirMined);
      if (statsObj.elixirMinedHist.length > averageWindow)
        statsObj.elixirMinedHist.shift();
      if (this.turn % 10 == 0)
        statsObj.elixirIncomeDataset.push({
          x: this.turn,
          y: average(statsObj.elixirMinedHist),
        });
    }
