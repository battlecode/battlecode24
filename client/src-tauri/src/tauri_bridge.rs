...
const invokeArrayResult = (operation, ...args) =>
    __TAURI__.invoke('tauri_api', { operation, args })

const invokeSingleResult = async (operation, ...args) => {
    return (await invokeArrayResult(operation, ...args))[0]
}
