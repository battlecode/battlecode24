...
export default class Stats {
  ...
  private islandCountDisplay: HTMLDivElement
  ...
  private initIslandCount(islands: number) {
    ...
    display.innerHTML = `<div style="color:var(--red)"> ${0} </div><div style="color:var(--blue)">${0} </div>`
  ...
  setIslandCount(t1: number, t2: number) {
    this.islandCountDisplay.innerHTML = `<div style="color:var(--red)"> ${t1} </div><div style="color:var(--blue)">${t2} </div>`
  }
  ...
  initializeGame(teamNames: Array<string>, teamIDs: Array<number>, islands: number) {
    ...
    let [wrapper, display] = this.initIslandCount(islands)
    this.div.append(wrapper)
    this.islandCountDisplay = display
    this.div.appendChild(document.createElement("hr"))
