...
export default class MapEditorForm {
  ...
  private currents: boolean[];
  private islands: boolean[];
  ...
  constructor(conf: Config, imgs: AllImages, canvas: HTMLCanvasElement) {
    ...
    // symmetry
    this.symmetryForm = new SymmetryForm(() => {this.initWalls(); this.initResources(); this.render()});
 ...
 */
   ...
   */
  private getActiveForm(): RobotForm | TileForm | ResourceForm {
    return (this.tilesRadio.checked ? this.tilesForm : (this.robotsRadio.checked ? this.robotsForm : this.resourcesForm))
  }
