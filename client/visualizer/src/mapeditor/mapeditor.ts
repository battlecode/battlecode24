...
/**
 * Allows the user to download a .${this.conf.map_extension} file representing the map generated
 * in the map editor.
 */
export default class MapEditor {
  ...
  private basediv(): HTMLDivElement {
    ...
    helpDiv.innerHTML = `<b class="blue">Keyboard Shortcuts (Map Editor)</b><br>
      ...
      <br>
      To place headquarters, enter the "robots" mode, click to set the coordinates, and click "Add/Update" or "Delete." 
      <br>
      To set walls, enter "walls" mode, select brush size and brush style,
      ...
      map.) <br>
      <br>
      Exported file name must be the same as the map name chosen above. For instance, <code>DefaultMap.${this.conf.game_extension}</code>.`
    ...
    button.onclick = () => {
      ...
      if (data) {
        if (process.env.ELECTRON && this.scaffold) {
          ...
          MapGenerator.exportFile(data, `${name}.${this.conf.map_extension}`)
        }
      }
    }
    return button
