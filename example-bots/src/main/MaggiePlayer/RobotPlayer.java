...
public class RobotPlayer {
    ...
    private static void runBot(RobotController rc) throws GameActionException {
        ...
        if(rc.getRoundNum() < 150){
            ...
            int k = rng.nextInt(3);
            if (k == 0 && rc.canBuild(TrapType.EXPLOSIVE, rc.getLocation())){
                rc.build(TrapType.EXPLOSIVE, rc.getLocation());
                System.out.println("explosive trap built");
            }
            else if (k == 1 && rc.canBuild(TrapType.WATER, rc.getLocation())){
                rc.build(TrapType.WATER, rc.getLocation());
                System.out.println("water trap built");
            }
            else if (k == 2 && rc.canBuild(TrapType.STUN, rc.getLocation())){
                rc.build(TrapType.STUN, rc.getLocation());
                System.out.println("stun trap built");
            }
            moveRandom(rc);
        }
