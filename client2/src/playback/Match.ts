...
export default class Match {
    ...
    constructor(
        ...
        this.deltas.forEach((delta, i) =>
            assert(
                delta.roundID() === i + 1,
                `Wrong turn ID: is ${delta.roundID()}, should be ${i}`
            )
        )
    ...
    public jumpToTurn(turnNumber: number): void {
        const snapshotIndex = Math.min(
            Math.floor(turnNumber / SNAPSHOT_EVERY),
        ...
        while (turn.turnNumber < turnNumber) {
            ...
            if (
                ...
                this.snapshots.length < turn.turnNumber / SNAPSHOT_EVERY
            ) {
